...
from .incremental_path import incremental_path

logger = get_logger()
BACK_UP_PATH = "D:/git/kaggle-Home-Credit-Default-Risk/neptune-features/"
BACK_UP_FHEADER = "features"
BACK_UP_DHEADER = "data"
...
class FeatureJoiner(BaseTransformer):
    ...
    def transform(self, numerical_feature_list, categorical_feature_list, **kwargs):
        ...
        info = "_".join(map(str, features.shape))
        fpath = incremental_path(BACK_UP_PATH,
                                 BACK_UP_FHEADER,
                                 info)
        dpath = incremental_path(BACK_UP_PATH,
                                 BACK_UP_DHEADER,
                                 info)

        np.save(fpath, list(features.columns))
        save_dataframe32(dpath, features)
        gc.collect()
