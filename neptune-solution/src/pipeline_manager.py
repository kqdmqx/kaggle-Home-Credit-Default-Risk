...
logger = init_logger()
ctx = neptune.Context()
params = read_params(ctx, fallback_file='D:/git/open-solution-home-credit/configs/neptune.yaml')
...
def train_evaluate_cv_first_level(pipeline_name, dev_mode):
    ...
    logger.info("app_tr:{}".format(tables.application_train.shape))
    logger.info("app_ts:{}".format(tables.application_test.shape))
    # logger.info("HERE1")
    # time.sleep(5)
    # return
    fold_scores = []
...
BACK_UP_PATH = "D:/git/kaggle-Home-Credit-Default-Risk/neptune-features/"
...
def train_evaluate_predict_cv_first_level(pipeline_name, dev_mode, submit_predictions):
    ...
    for fold_id, (train_idx, valid_idx) in enumerate(fold_generator):
        train_fn = BACK_UP_PATH + "train_idx_{}.npy".format(fold_id)
        valid_fn = BACK_UP_PATH + "valid_idx_{}.npy".format(fold_id)

        np.save(train_fn, train_idx)
        np.save(valid_fn, valid_idx)
...
def _read_data_bak(dev_mode, read_train=True, read_test=False):
    logger.info('Reading data...')
    if dev_mode:
...
def _read_data(dev_mode, read_train=True, read_test=False):
    ...
    if read_train:
        raw_data['application_train'] = load_dataframe32(params.train_binfilepath, nrows)

    if read_test:
        raw_data['application_test'] = load_dataframe32(params.test_binfilepath, nrows)

    raw_data['bureau'] = load_dataframe32(params.bureau_binfilepath, nrows)
    raw_data['credit_card_balance'] = load_dataframe32(params.credit_card_balance_binfilepath, nrows)
    raw_data['pos_cash_balance'] = load_dataframe32(params.POS_CASH_balance_binfilepath, nrows)
    raw_data['previous_application'] = load_dataframe32(params.previous_application_binfilepath, nrows)
    raw_data['bureau_balance'] = load_dataframe32(params.bureau_balance_binfilepath, nrows)
    raw_data['installments_payments'] = load_dataframe32(params.installments_payments_binfilepath, nrows)
